...
| `422` | **Permanent — do not retry.** The audio is **undecodable/unprocessable** (corrupt, truncated, unsupported, or no audio in the window); the `error` message (`"unprocessable audio: …"`) carries the human reason. *(Before v0.6.1, `422` also covered an "ambiguous folder" case; the attribution path now resolves any path to a single book — changelog #9 — so that no longer occurs.)* |
...
1. **Extraction — blind.** Transcribe the audio — the head intro window, plus the **tail** when the head carries no complete credit (Audible & co. announce the book at the *end*) — then the LLM extracts title/author/narrator **without ever seeing `expected`.** Each detected field must be backed by a verbatim transcript span or it's nulled (evidence-guard). *Don't lead the witness:* if extraction saw the metadata it would hallucinate hearing exactly the tags, destroying earwitness's independence. Output (`detection`) is **frozen fact.**
